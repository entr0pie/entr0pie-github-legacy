...
We can choose a search engine and do a search in it. If we pay attention, there's a program name in the footer of the page called <b>Searchor</b>. 
...
Luck or coincidence (probably the last), this is the code that will let us inside.

## How to inject code?

Eval is a function that allow to execute strings like code. But are some restrictions to use it. One of them is that the expression inside can't contain more than one line of code (even with "\n" or ";"). The other is that native functions like `import` must be called with the [built-in original sintax](https://docs.python.org/3/reference/import.html#the-import-system), like:
...
```
That way, our payload will be executed on the server side:

```python
' + str(__import__('os').system('COMMAND')))#
```

My way to get a reverse shell was through telnet:
...
We login as the `svc` user. The user flag is in the home directory. 

Starting our searching in the system, we can found some sensitive archives. The first one is `/etc/hosts`, that informs us about a _gitea_ subdomain (gitea.searcher.htb). 
...
Acessing the Gitea homepage, we found that there's a `administrator` user that contributed to the Searcher_site repository.
...
There's it! The gitea service runs via docker on the machine. We can inspect it through the [docker-inspect](https://docs.docker.com/engine/reference/commandline/inspect/#get-a-subsection-in-json-format) command. Inside the container configuration, we found a database password that matches with the administrator's password too!

![Administrator password](/assets/writeups/2023-05-01-busqueda/docker_config.png)

## Bunker source code. 

Inside the gitea, we found a _scripts_ repo. Inside we have the `system-checkup.py`.
