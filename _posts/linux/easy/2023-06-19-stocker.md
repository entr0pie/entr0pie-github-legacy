...
1. The attacker finds a web server and an SSH.
2. Enumerating the web server, the attacker finds the subdomain `dev`.
3. In it, the hacker comes across a login page, with the backend in ExpressJS.
4. They bypass authentication through NoSQL injection.
5. The attacker discovers a common item store. The page communicates with the server through the `/api` route.
6. The attacker injects HTML code into the checkout invoice, gaining access to local system files (LFI). They enumerate the system users (`root` and `angoose`).
7. The attacker (through trial and error) discovers the file responsible for subdomain routing (`/var/www/dev/index.js`).
8. The file contains a password for the database, which matches the password of the user `angoose`.
9. Inside the server, the attacker has permissions to run the following command as `root`: `sudo /usr/bin/node /usr/local/scripts/*.js`
10. The attacker escalates privileges using path traversal (`sudo /usr/bin/node /usr/local/scripts/../../../home/angoose/malicious.js`)
...
<!-- For more detailed information on how this vulnerability works, please check "[[#Appendix A: How did we bypass the login page?]]". -->
